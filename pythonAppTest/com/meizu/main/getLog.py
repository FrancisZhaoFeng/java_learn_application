...
# 请配置下面的3个全局变量
BASE_PATH = "/home/dsh/"
BASE_READ = "flyme_app/flyme4_M86_iof"
LOG_PATH = BASE_PATH + "share_122/log_flyme4_M86_S6_iof/"
INIT_PATH = BASE_PATH + "apptest/com/meizu/tools/"
APK_PATH = BASE_PATH + BASE_READ
INFO_PATH = BASE_PATH + "share_122/flyme4_M86_S6_iof"
...
#         file_list.sort(cmp=lambda s1, s2:cmp(s1[0], s2[1]))
    ...
    def get_cmd_pkg_activity(self, filepath):
        ...
        return self.package+"/"+activity_name
        ...
        for filepath in apk_path:
            ...
            if self.package:
                ...
                if pkg.find("Success") >= 0:
                    ...
                    time.sleep(10)
                    act = os.popen("adb shell dumpsys window w | grep mCurrent").read()
                    print "打开信息:%s" % act
                    if act.find(self.package) >= 0:
                        print "打开成功(保存数据)：%s" % filepath
